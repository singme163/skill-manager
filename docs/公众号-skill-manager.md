# 公众号文章草稿：Skill Manager v2.2

> 使用说明：
> - 标题三选一（或自行组合）
> - 五张配图已生成在 docs/images/ 下（main / discovery / lint / translate / sync），上传公众号编辑器即可；想重新生成跑 `Scripts/snapshots.sh`
...
于是我做了 **Skill Manager**——一个 macOS 原生 App。三个月前它还只是个"增删查"小工具，现在迭代到 v2.2，已经长成一个完整的 **skill 工作台**：发现、管理、写好、看懂、同步，一站式。

![主界面：三栏视图，多来源合并管理](images/main.png)
...
![发现页：精选索引一键安装](images/discovery.png)
...
![检查标签：Skill Lint 实时体检](images/lint.png)

### 英文 skill 看不懂？一键变中文

社区里的好 skill 十有八九是英文写的。v2.2 加了**一键翻译**：

- 详情页描述旁、预览页右上角各有一个「译」按钮，点一下整篇 SKILL.md 变中文，再点切回原文
- **保留 Markdown 排版**：标题、列表、引用标记原样不动，代码块跳过不翻，中英混排的文档只翻需要翻的行
- 用的是 **macOS 系统离线翻译**（需 macOS 15+）——不走任何云端 API，内容不出本机，首次使用系统会提示下载语言包，之后完全离线
- 译文按 skill 缓存，来回切换零等待

![预览翻译：整篇英文 SKILL.md 一键变中文，排版原样保留](images/translate.png)
...
![设置：工具管理 / Token / 多机同步 / 菜单栏](images/sync.png)
...
- MIT 开源，46 个单元测试，每次发版 CI 全绿才出包

---

### 彩蛋：这个 App 是怎么长大的

坦白说：从 PRD 到 v2.2 的每一行代码、每一个测试、连 CI/CD 流水线、这篇文章甚至文中的截图，都是我和 Claude Code 结对完成的。

节奏大概是：先让它写 PRD，我拍板技术选型和功能边界；然后 v1.0 一个下午出可用版本；之后每说一句"继续迭代下一版"，它就按路线图交付一个版本——工具可配置化、项目级支持、发现与更新、编辑质量工具、分享与同步、离线翻译，从 v1.0 到 v2.2 八个版本，一路发到 GitHub Release。
